...
const InstructorResults = () => {
  ...
  const fetchResults = async (selectedCourseId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:5000/api/assessments/instructor/course/${selectedCourseId}/results`, {
        headers: { Authorization: `Bearer ${token}` },
      });
          ...
          <div className="p-4 md:p-6">
                      ...
                      {results.map((result, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="p-3 text-gray-600">{result.assessmentId?.assessmentNumber}</td>
            ...
            {!loading && courseId && results.length === 0 && (
              <div className="bg-white rounded-lg border border-gray-200 p-6 text-center shadow-md">
                ...
                <p className="text-gray-500">There are no assessment results available for this course.</p>
              </div>
            )}
          </div>
        </div>
